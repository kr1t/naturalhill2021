...
<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">{{clean( trans('niva-backend.edit_slider') , array('Attr.EnableID' => true))}}
    </h1>

    <!-- DataTales Example -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{clean( trans('niva-backend.edit_slider') ,
                array('Attr.EnableID' => true))}}</h6>
        </div>
        <div class="card-body">

            <a href="{{route('slider.index') . '?language=' . request()->input('language')}}"
                class="btn btn-primary btn-back">{{clean( trans('niva-backend.back_slider') , array('Attr.EnableID' =>
                true))}} </a>
                    ...
                    <form action="{{route('slider.update', $slider->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <strong>{{clean( trans('niva-backend.heading_1') , array('Attr.EnableID' =>
                                                true))}}</strong>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <strong>{{clean( trans('niva-backend.heading_2') , array('Attr.EnableID' =>
                                                true))}}</strong>
                                ...
                                <div class="form-group">
                                    <strong>{{clean( trans('niva-backend.typed_text') , array('Attr.EnableID' =>
                                        true))}}</strong>
                                    <input type="text" name="typed_text" class="form-control"
                                        value="{{$slider->typed_text}}">
                                </div>

                                <div class="form-group">
                                    <strong>{{clean( trans('niva-backend.body') , array('Attr.EnableID' =>
                                        true))}}</strong>
                                    <textarea name="bodyslider" class="form-control" id="bodyslider"
                                        rows="10">{{$slider->bodyslider}}</textarea>
                                </div>
